...
rpcpassword=$(cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 32 | head -n 1)
 ...
 #Installing Daemon
 cd ~
wget https://github.com/AXErunners/axe/releases/download/v1.1.8/axecore-1.1.8-x86_64-linux-gnu.tar.gz
tar -xzf axecore-1.1.8-x86_64-linux-gnu.tar.gz -C ~/AXE-MN-setup
rm -rf axecore-1.1.8-x86_64-linux-gnu.tar.gz

 
 stop_daemon
 
 # Deploy binaries to /usr/local/bin
 sudo rm ~/AXE-MN-etup/axecore-1.1.8/bin/axe-qt
 sudo rm ~/AXE-MN-etup/axecore-1.1.8/bin/test*
 sudo cp ~/AXE-MN-etup/axecore-1.1.8/bin/axe* /usr/local/bin/
